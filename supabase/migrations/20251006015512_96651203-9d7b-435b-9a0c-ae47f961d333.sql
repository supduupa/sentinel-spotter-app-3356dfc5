...
-- Skip type creation if it exists, create tables and functions
...
CREATE TABLE IF NOT EXISTS public.galamsey_reports (
    id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
    date DATE NOT NULL,
    updated_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(),
    user_id UUID,
...
DROP TRIGGER IF EXISTS update_galamsey_reports_updated_at ON public.galamsey_reports;
...
-- 9. Enable realtime for galamsey_reports
